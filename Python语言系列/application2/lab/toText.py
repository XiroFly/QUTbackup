...
def merge_files(folder_path, output_file):
    ...
    with open(output_file, mode='a+', encoding='utf-8') as out_file:
        ...
        for f in file_list:
            ...
            # 如果是代码文件，则添加到txt中（此处如果有其他类型的文件也可以修改）
            #elif f.endswith('.py') or f.endswith('.cpp') or f.endswith('.c') or f.endswith('.java') or f.endswith('.xml'):
            elif f.endswith(".cs"):
               ...
               # filename = os.path.splitext(f)[0]
                ...
                # 添加项目路径文件名和分隔符到txt中 
                global margin
                relative_path=os.path.relpath(file_path, margin)
...
if __name__ == '__main__':
    if os.getpid() == 0:
        ...
        print("当前程序未以系统权限运行")
    # 设置输入文件夹路径（注意使用原始字符串）
    folder_path = r'G:\备份\作业\软件设计与体系结构\src'
    margin=folder_path
    # 设置输出txt文件名
    ...
    #D:\Users\fly\Python\
    #https://convertio.co/zh/txt-pdf/ 转换为pdf
    #https://www.chatpdf.com/ 分析PDf